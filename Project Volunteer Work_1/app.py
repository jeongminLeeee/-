...
def my_schedule():
    ...
    for p in personal:
        ...
        if start_date <= today <= end_date:
            ...
            d_day = (start_date - today).days   # 미래 → +
            status = 1

        else:
            d_day = -(today - end_date).days    # 🔥 과거 → - 로 변경
            status = 2   # 끝남
...
def update_home():
    ...
    # 기본 구조 보장
    home.setdefault("notice", "")
    home.setdefault("intro", "")
    home.setdefault("intro_image", "")
    ...
    # -----------------------------
    if section == "notice":
        home["notice"] = data.get("value", "")

    # -----------------------------
    # 소개
    # -----------------------------
    elif section == "intro":
        home["intro"] = data.get("value", "")

    # -----------------------------
    # 소개 이미지
    # -----------------------------
    elif section == "intro_image":
        home["intro_image"] = data.get("image", "")

    # -----------------------------
    # 🔥 파트 추가 (ID 꼬임 방지)
    # -----------------------------
    elif section == "add_part":
        parts = home["parts"]

        if parts:
            new_id = str(max(map(int, parts.keys())) + 1)
        else:
            new_id = "1"
    ...
    # 🔥 파트 삭제
    # -----------------------------
    elif section.startswith("delete_part_"):
        pid = section.replace("delete_part_", "")

        if pid in home["parts"]:
            del home["parts"][pid]

    # -----------------------------
    # 🔥 파트 수정
    # -----------------------------
    elif section.startswith("part_"):
        pid = section.replace("part_", "")
...
def upload_image():
    ...
    if "file" not in request.files:
        return jsonify({"success": False, "msg": "파일 없음"})
    ...
    if file and allowed_file(file.filename):
        ...
        # 🔥 중복 방지용 파일명
        filename = str(int(datetime.datetime.now().timestamp())) + "_" + filename
    ...
    return jsonify({"success": False, "msg": "허용되지 않는 파일"})
...
def tasks_page():
    ...
    if grade and grade != "전체":
        filtered_tasks = [t for t in filtered_tasks if str(t.get("grade")) == grade]

    if class_num and class_num != "전체":
        filtered_tasks = [t for t in filtered_tasks if str(t.get("class")) == class_num]
    ...
    # ✔ 마감순
    if sort == "deadline":
        safe_tasks.sort(key=lambda x: x["d_day"])

    # ✔ 이름순
    elif sort == "name":
        safe_tasks.sort(key=lambda x: x.get("subject", ""))
    ...
    return render_template("tasks.html", tasks=safe_tasks)
...
def admin_add_task():
    ...
    for f in files:
        if f:
            filename = f"{datetime.datetime.now().timestamp()}_{f.filename}"
...
def admin_delete_task():
    ...
    for t in tasks:
        ...
        if str(t.get("id")) == task_id:
            # 🔥 이미지 파일도 삭제
            for img in t.get("imgs", []):
                img_path = img.replace("/static/", "")
    ...
    # 🔥 유저 캘린더에서도 제거
    if os.path.exists(USER_CAL_FILE):
        with open(USER_CAL_FILE, "r", encoding="utf-8") as f:
            user_tasks = json.load(f)
...
def delete_item():
    ...
    # 🔥 댓글도 같이 삭제
    for u in chat:
        for cid, parent, content in chat[u]:
            if str(parent) == target_id:
                delete_ids.add(cid)

    delete_ids.add(int(target_id))
        ...
        for cid, parent, content in chat[u]:
            ...
            if user == "admin":
                if cid not in delete_ids:
                    new_list.append([cid, parent, content])
...
# 🔥 관리자 행사 추가 (수정본)
# -----------------------------
@app.route("/admin/add_school_event", methods=["POST"])
def admin_add_school_event():
    ...
    for file in files:
        if file and allowed_file(file.filename):
            ...
            save_path = os.path.join("static/uploads", filename)
            file.save(save_path)
            img_paths.append("/static/uploads/" + filename)
...
def admin_delete_event():
    ...
    for user in event_data:
        event_data[user] = [
            str(eid)
            for eid in event_data[user]
            if str(eid) != event_id
        ]

    save_event_calendar(event_data)
